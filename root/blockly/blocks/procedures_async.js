...
/**
 * @fileoverview Async Procedure blocks for Blockly.
 * @suppress {checkTypes|visibility}
 */
'use strict';

goog.module('Blockly.libraryBlocks.AsyncProcedures');

/* eslint-disable-next-line no-unused-vars */
const AbstractEvent = goog.requireType('Blockly.Events.Abstract');
const ContextMenu = goog.require('Blockly.ContextMenu');
const Events = goog.require('Blockly.Events');
const AsyncProcedures = goog.require('Blockly.AsyncProcedures');
 ...
 */
const ASYNC_PROCEDURE_DEF_COMMON = {
  /**
  ...
  setStatements_: function(hasStatements) {
    ...
    if (hasStatements) {
      this.appendStatementInput('STACK').appendField(
          Msg['ASYNC_PROCEDURES_DEFNORETURN_DO']);
  ...
  updateParams_: function() {
    ...
    if (this.arguments_.length) {
      paramString =
          Msg['ASYNC_PROCEDURES_BEFORE_PARAMS'] + ' ' + this.arguments_.join(', ');
    }
  ...
  domToMutation: function(xmlElement) {
    ...
    this.updateParams_();
    AsyncProcedures.mutateCallers(this);
  ...
  decompose: function(workspace) {
    ...
    containerBlockNode.setAttribute('type', 'async_procedures_mutatorcontainer');
    ...
    for (let i = 0; i < this.arguments_.length; i++) {
      ...
      argBlockNode.setAttribute('type', 'async_procedures_mutatorarg');
    ...
    if (this.type === 'async_procedures_defreturn') {
  ...
  displayRenamedVar_: function(oldName, newName) {
    ...
    if (this.mutator && this.mutator.isVisible()) {
      ...
      for (let i = 0, block; (block = blocks[i]); i++) {
        if (block.type === 'async_procedures_mutatorarg' &&
  ...
  customContextMenu: function(options) {
    ...
    option.text = Msg['ASYNC_PROCEDURES_CREATE_DO'].replace('%1', name);
...
blocks['async_procedures_defnoreturn'] = {
   ...
   */
  init: function() {
    const initName = AsyncProcedures.findLegalName('', this);
    const nameField = new FieldTextInput(initName, Procedures.rename);
    nameField.setSpellcheck(false);
    this.appendDummyInput()
        .appendField(Msg['ASYNC_PROCEDURES_DEFNORETURN_TITLE'])
        .appendField(nameField, 'NAME')
        .appendField('', 'PARAMS');
    this.setMutator(new Mutator(['async_procedures_mutatorarg']));
    if ((this.workspace.options.comments ||
         (this.workspace.options.parentWorkspace &&
          this.workspace.options.parentWorkspace.options.comments)) &&
        Msg['ASYNC_PROCEDURES_DEFNORETURN_COMMENT']) {
      this.setCommentText(Msg['ASYNC_PROCEDURES_DEFNORETURN_COMMENT']);
    }
    this.setStyle('async_procedure_blocks');
    this.setTooltip(Msg['ASYNC_PROCEDURES_DEFNORETURN_TOOLTIP']);
    this.setHelpUrl(Msg['ASYNC_PROCEDURES_DEFNORETURN_HELPURL']);
    this.arguments_ = [];
    this.argumentVarModels_ = [];
    this.setStatements_(true);
    this.statementConnection_ = null;
   ...
   * @this {Block}
   */
  getAsyncProcedureDef: function() {
    return [this.getFieldValue('NAME'), this.arguments_, false];
  },
  callType_: 'async_procedures_callnoreturn',
};

blocks['async_procedures_defreturn'] = {
  ...ASYNC_PROCEDURE_DEF_COMMON,
  /**
  ...
  init: function() {
    const initName = AsyncProcedures.findLegalName('', this);
    const nameField = new FieldTextInput(initName, AsyncProcedures.rename);
    nameField.setSpellcheck(false);
    this.appendDummyInput()
        .appendField(Msg['ASYNC_PROCEDURES_DEFRETURN_TITLE'])
        .appendField(nameField, 'NAME')
        .appendField('', 'PARAMS');
    this.appendValueInput('RETURN')
        .setAlign(Align.RIGHT)
        .appendField(Msg['ASYNC_PROCEDURES_DEFRETURN_RETURN']);
    this.setMutator(new Mutator(['async_procedures_mutatorarg']));
    if ((this.workspace.options.comments ||
         (this.workspace.options.parentWorkspace &&
          this.workspace.options.parentWorkspace.options.comments)) &&
        Msg['ASYNC_PROCEDURES_DEFRETURN_COMMENT']) {
      this.setCommentText(Msg['ASYNC_PROCEDURES_DEFRETURN_COMMENT']);
    }
    this.setStyle('async_procedure_blocks');
    this.setTooltip(Msg['ASYNC_PROCEDURES_DEFRETURN_TOOLTIP']);
    this.setHelpUrl(Msg['ASYNC_PROCEDURES_DEFRETURN_HELPURL']);
    this.arguments_ = [];
    this.argumentVarModels_ = [];
    this.setStatements_(true);
    this.statementConnection_ = null;
  ...
  callType_: 'async_procedures_callreturn',
};

blocks['async_procedures_mutatorcontainer'] = {
  /**
  ...
  init: function() {
    this.appendDummyInput().appendField(
        Msg['ASYNC_PROCEDURES_MUTATORCONTAINER_TITLE']);
    this.appendStatementInput('STACK');
    this.appendDummyInput('STATEMENT_INPUT')
        .appendField(Msg['ASYNC_PROCEDURES_ALLOW_STATEMENTS'])
        .appendField(new FieldCheckbox('TRUE'), 'STATEMENTS');
    this.setStyle('async_procedure_blocks');
    this.setTooltip(Msg['ASYNC_PROCEDURES_MUTATORCONTAINER_TOOLTIP']);
    this.contextMenu = false;
  },
};

blocks['async_procedures_mutatorarg'] = {
  /**
   * Mutator block for procedure argument.
   * @this {Block}
   */
  init: function() {
    const field = new FieldTextInput(AsyncProcedures.DEFAULT_ARG, this.validator_);
    ...
    this.appendDummyInput()
        .appendField(Msg['ASYNC_PROCEDURES_MUTATORARG_TITLE'])
        .appendField(field, 'NAME');
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setStyle('async_procedure_blocks');
    this.setTooltip(Msg['ASYNC_PROCEDURES_MUTATORARG_TOOLTIP']);
    this.contextMenu = false;
 ...
 */
const ASYNC_PROCEDURE_CALL_COMMON = {
  /**
   * Returns the name of the procedure this block calls.
   * @return {string} Procedure name.
   * @this {Block}
   */
  getAsyncProcedureCall: function() {
   ...
   * @this {Block}
   */
  renameAsyncProcedure: function(oldName, newName) {
    if (Names.equals(oldName, this.getAsyncProcedureCall())) {
      this.setFieldValue(newName, 'NAME');
      const baseMsg = this.outputConnection ?
          Msg['ASYNC_PROCEDURES_CALLRETURN_TOOLTIP'] :
          Msg['ASYNC_PROCEDURES_CALLNORETURN_TOOLTIP'];
   ...
   * @this {Block}
   */
  setAsyncProcedureParameters_: function(paramNames, paramIds) {
    // Data structures:
    // this.arguments = ['x', 'y']
    ...
    const defBlock =
        AsyncProcedures.getDefinition(this.getAsyncProcedureCall(), this.workspace);
    const mutatorOpen =
    ...
    if (topRow) {
      if (this.arguments_.length) {
        if (!this.getField('WITH')) {
          topRow.appendField(Msg['ASYNC_PROCEDURES_CALL_BEFORE_PARAMS'], 'WITH');
          topRow.init();
        }
      } else {
        if (this.getField('WITH')) {
  ...
  mutationToDom: function() {
    const container = xmlUtils.createElement('mutation');
    container.setAttribute('name', this.getAsyncProcedureCall());
    for (let i = 0; i < this.arguments_.length; i++) {
      const parameter = xmlUtils.createElement('arg');
  ...
  domToMutation: function(xmlElement) {
    ...
    this.renameAsyncProcedure(this.getAsyncProcedureCall(), name);
    const args = [];
    const paramIds = [];
    for (let i = 0, childNode; (childNode = xmlElement.childNodes[i]); i++) {
      if (childNode.nodeName.toLowerCase() === 'arg') {
    ...
    this.setAsyncProcedureParameters_(args, paramIds);
  },
  /**
   * Returns the state of this block as a JSON serializable object.
  ...
  saveExtraState: function() {
    const state = Object.create(null);
    state['name'] = this.getAsyncProcedureCall();
    if (this.arguments_.length) {
   ...
   */
  loadExtraState: function(state) {
    this.renameAsyncProcedure(this.getAsyncProcedureCall(), state['name']);
    const params = state['params'];
    if (params) {
      const ids = [];
      ids.length = params.length;
      ids.fill(null);
      this.setAsyncProcedureParameters_(params, ids);
    }
  },
  /**
   * Return all variables referenced by this block.
  ...
  onchange: function(event) {
    ...
    if (event.type === Events.BLOCK_CREATE &&
      ...
      const name = this.getAsyncProcedureCall();
      let def = AsyncProcedures.getDefinition(name, this.workspace);
      if (def &&
      ...
      if (!def) {
        ...
        const callName = this.getAsyncProcedureCall();
        const newName = AsyncProcedures.findLegalName(callName, this);
        if (callName !== newName) {
          this.renameAsyncProcedure(callName, newName);
        }
      ...
      // the orphan.
      const name = this.getAsyncProcedureCall();
      const def = AsyncProcedures.getDefinition(name, this.workspace);
      if (!def) {
        Events.setGroup(event.group);
        this.dispose(true);
        Events.setGroup(false);
  ...
  customContextMenu: function(options) {
    ...
    const option = {enabled: true};
    option.text = Msg['ASYNC_PROCEDURES_HIGHLIGHT_DEF'];
    const name = this.getAsyncProcedureCall();
    const workspace = this.workspace;
    option.callback = function() {
      const def = AsyncProcedures.getDefinition(name, workspace);
      if (def) {
...
blocks['async_procedures_callnoreturn'] = {
  ...ASYNC_PROCEDURE_CALL_COMMON,
  /**
  ...
  init: function() {
    ...
    this.setHelpUrl(Msg['ASYNC_PROCEDURES_CALLNORETURN_HELPURL']);
    this.arguments_ = [];
    this.argumentVarModels_ = [];
    this.quarkConnections_ = {};
    this.quarkIds_ = null;
  ...
  defType_: 'async_procedures_defnoreturn',
};

blocks['async_procedures_callreturn'] = {
  ...ASYNC_PROCEDURE_CALL_COMMON,
  /**
  ...
  init: function() {
    ...
    this.setHelpUrl(Msg['ASYNC_PROCEDURES_CALLRETURN_HELPURL']);
    this.arguments_ = [];
    this.argumentVarModels_ = [];
    this.quarkConnections_ = {};
    this.quarkIds_ = null;
  ...
  defType_: 'async_procedures_defreturn',
};

blocks['async_procedures_ifreturn'] = {
  /**
  ...
  init: function() {
    ...
    this.appendValueInput('VALUE').appendField(
        Msg['ASYNC_PROCEDURES_DEFRETURN_RETURN']);
    this.setInputsInline(true);
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setStyle('async_procedure_blocks');
    this.setTooltip(Msg['ASYNC_PROCEDURES_IFRETURN_TOOLTIP']);
    this.setHelpUrl(Msg['ASYNC_PROCEDURES_IFRETURN_HELPURL']);
  ...
  onchange: function(e) {
    ...
    if (legal) {
      ...
      if (block.type === 'async_procedures_defnoreturn' && this.hasReturnValue_) {
        this.removeInput('VALUE');
        this.appendDummyInput('VALUE').appendField(
            Msg['ASYNC_PROCEDURES_DEFRETURN_RETURN']);
        this.hasReturnValue_ = false;
      } else if (
          block.type === 'async_procedures_defreturn' && !this.hasReturnValue_) {
        this.removeInput('VALUE');
        this.appendValueInput('VALUE').appendField(
            Msg['ASYNC_PROCEDURES_DEFRETURN_RETURN']);
        this.hasReturnValue_ = true;
      ...
      this.setWarningText(Msg['ASYNC_PROCEDURES_IFRETURN_WARNING']);
    }
    if (!this.isInFlyout) {
   ...
   */
  FUNCTION_TYPES: ['async_procedures_defnoreturn', 'async_procedures_defreturn'],
};
