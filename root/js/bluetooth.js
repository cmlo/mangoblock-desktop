...
class Bluetooth {
    ...
    webConnect = async () => {
        navigator.bluetooth.ondisconnect = () => {
        ...
        try {
            this.port = await navigator.bluetooth.requestPort();
        } catch(e) {
    ...
    electronConnect = async (portName = "", autoConnect = false, uploadMode = false) => {
        ...
        NotifyS(Blockly.Msg.UI_SERIAL_CONNECTED);
        statusbar.log(Blockly.Msg.UI_SERIAL_CONNECTED);
        statusbar.port(Blockly.Msg.UI_USB_CONNECTED + ` (${portName})`)
    ...
    boardReset = (enterToBootMode) => { // Hard-reset
