...
const clearInputBuffer = () => inputBuffer.splice(0, inputBuffer.length); // empty inputBuffer
...
class EspLoader {
  ...
  async connect() {
    clearInputBuffer();
    await boardReset(true); // Enter to BOOT mode

    return String.fromCharCode.apply(null, inputBuffer).indexOf("waiting for download") >= 0;
   ...
   */
  async disconnect() {
    boardReset(false); // Exit BOOT mode
    /*
    if (reader) {
      await reader.cancel();
      reader = null;
...
/**
  ...
  /*async flashData(binaryData, offset=0, part=0) {
    ...
    const bbin = Array.from(new Uint8Array(binaryData));
    ...
    while ((filesize - position) > 0) {
      ...
      if ((filesize - position) >= flashWriteSize) {
        if (!isElectron) {
          ...
          block = bbin.slice(position, position + flashWriteSize);
          console.log("Block", position, position + flashWriteSize);
        }
      } else {
        // Pad the last block
        if (!isElectron) {
          ...
          block = bbin.slice(position, filesize);
        }
  ...
  async getStubCode() {
    ...
    if (!isElectron) {
      ...
      stubcode = await response.json();
    } else {
      stubcode = JSON.parse(await readFileAsync(`${sharedObj.rootPath}/stubs/${this.getStubFile()}.json`));
    }
