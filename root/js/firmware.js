...
let firmwareUpgradeFlow = async () => {
    ...
    if ((!isElectron) && (board?.chip === "ESP32")) {
        ...
        windowFirmwareUpdate = window.open(
            "/firmware.html?board=" + encodeURI(settings.boardId) + "&firmware=" + encodeURI(JSON.stringify(board.firmware)),
            "Firmware Update",
        ...
        if (board?.chip === "RP2") {
            ...
            const checkRP2DriveAvailable = async () => {
                ...
                const RP2DriveInfo = drives.find(a => a.filesystem === "Removable Disk");
                if (RP2DriveInfo) {
...
$("#install-firmware-button").click(async () => {
    ...
    let fwPath = board.firmware[0].path;
    if (!isElectron) {
        fwPath = fwPath.startsWith("/") ? fwPath : `/boards/${settings.boardId}/${fwPath}`;
    } else {
        fwPath = fwPath.startsWith("/") ? sharedObj.rootPath + fwPath : `${sharedObj.rootPath}/boards/${settings.boardId}/${fwPath}`;
    ...
    if (chipId === "ESP32") { // ESP32
        if (!isElectron) {
            ...
            esptool.on("exit", (code) => {
                ...
                if (code === 0) {
                    $("#firmware-upgrade-dialog article.done .icon.success").show();
                    $("#firmware-upgrade-status").text(Blockly.Msg["UI_FirmwareUpgradeSuccessful"]);
                    $("#firmware-upgrade-dialog .upload-btn").show();
                } else {
                    $("#firmware-upgrade-dialog article.done .icon.fail").show();
                    $("#firmware-upgrade-status").text(Blockly.Msg["UI_FirmwareUpgradeFail"] + code);
                    $("#firmware-upgrade-dialog .upload-btn").hide();
                }
        ...
        if (!isElectron) {
            ...
            nodeFS.stat(sourceFile, function(err, stat) {
                ...
                readStream.on('data', function(buffer){
                    ...
                    $("#firmware-upgrade-dialog .progress-box > .caption").text(`${percentage}% (${bytesCopied} / ${filesize})`);
                    $("#firmware-upgrade-dialog .progress-box > .back-drop").width(`${percentage}%`);
                });
...
// setTimeout(() => firmwareUpgradeFlow(), 500);
