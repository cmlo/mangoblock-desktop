...
const MAXIMUM_TERMINAL_WIDTH = 600;

let terminalFullSizeFlag = false;

let setTerminalSize = function (size) {
    localStorage.setItem("terminal_size", size);
}
let getTerminalSize = function () {
    return localStorage.getItem("terminal_size");
}
let removeTerminalSize = function () {
...
if (deviceMode === MODE_REAL_DEVICE) {
    ...
    if (terminal_size) {
        ...
        beforeWidthTerminalSize = terminal_size >= MAXIMUM_TERMINAL_WIDTH ? MAXIMUM_TERMINAL_WIDTH : terminal_size;
        $(() => $("#open-terminal").click());
    }
}
...
$("#resize-terminal").click(() => {
    ...
    fitAddon.fit();
    setTerminalSize($("#terminal").width());
});
